...
//                    return handleAlignmentFault(far, elr);
                }
    ...
    private static boolean isVectorReg(Arm64.Operand operand) {
        if (operand.type != Arm64_const.ARM64_OP_REG) {
            throw new IllegalArgumentException("type=" + operand.type);
        }
        Arm64.OpValue value = operand.value;
        return value.reg >= Arm64_const.ARM64_REG_Q0 && value.reg <= Arm64_const.ARM64_REG_Q31;
    }

    private boolean handleAlignmentFault(long vaddr, long elr) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("handleAlignmentFault vaddr=0x" + Long.toHexString(vaddr) + ", elr=0x" + Long.toHexString(elr) + ", asm=" + insn.mnemonic + " " + insn.opStr);
        }
        ...
        switch (insn.mnemonic) {
            case "ldrh": {
                ...
                reg_write(operand.value.reg, pointer.getShort(0) & 0xffffffffL);
                Arm64.Operand mem = opInfo.op[1];
                Arm64.Operand imm = opInfo.op[2];
                reg_write(mem.value.reg, vaddr + imm.value.imm);
            ...
            case "ldr": {
                ...
                Arm64.Operand operand = opInfo.op[0];
                if (operand.value.reg >= Arm64_const.ARM64_REG_Q0 && operand.value.reg <= Arm64_const.ARM64_REG_Q31) {
                    reg_write_vector(operand.value.reg, pointer.getByteArray(0, 16));
                    Arm64.Operand mem = opInfo.op[1];
                    Arm64.Operand imm = opInfo.op[2];
                    reg_write(mem.value.reg, vaddr + imm.value.imm);
                    hypervisor.reg_set_elr_el1(elr + 4);
                    return true;
                } else if(is64BitReg(operand)) {
                    reg_write(operand.value.reg, pointer.getLong(0));
                    Arm64.Operand mem = opInfo.op[1];
                    Arm64.Operand imm = opInfo.op[2];
                    reg_write(mem.value.reg, vaddr + imm.value.imm);
                    hypervisor.reg_set_elr_el1(elr + 4);
                    ...
                    reg_write(operand.value.reg, pointer.getInt(0) & 0xffffffffL);
                    Arm64.Operand mem = opInfo.op[1];
                    Arm64.Operand imm = opInfo.op[2];
                    reg_write(mem.value.reg, vaddr + imm.value.imm);
                    hypervisor.reg_set_elr_el1(elr + 4);
            ...
            case "str": {
                ...
                Arm64.Operand operand = opInfo.op[0];
                if (isVectorReg(operand)) {
                    byte[] vector = reg_read_vector(operand.value.reg);
                    pointer.write(0, vector, 0, vector.length);
                    Arm64.Operand mem = opInfo.op[1];
                    Arm64.Operand imm = opInfo.op[2];
                    reg_write(mem.value.reg, vaddr + imm.value.imm);
                    hypervisor.reg_set_elr_el1(elr + 4);
            ...
            case "ldp": {
                ...
                if (is64BitReg(o1) && is64BitReg(o2)) {
                    reg_write(o1.value.reg, pointer.getLong(0));
                    reg_write(o2.value.reg, pointer.getLong(8));
                } else if (isVectorReg(o1) && isVectorReg(o2)) {
                    byte[] v1 = pointer.getByteArray(0, 16);
                    byte[] v2 = pointer.getByteArray(16, 16);
                    reg_write_vector(o1.value.reg, v1);
                    reg_write_vector(o2.value.reg, v2);
                } else {
                    throw new UnsupportedOperationException();
                }
                hypervisor.reg_set_elr_el1(elr + 4);
            ...
            case "stp": {
                ...
                Arm64.Operand o2 = opInfo.op[1];
                if (is64BitReg(o1) && is64BitReg(o2)) {
                    pointer.setLong(0, reg_read(o1.value.reg).longValue());
                    pointer.setLong(8, reg_read(o2.value.reg).longValue());
                } else if (isVectorReg(o1) && isVectorReg(o2)) {
                    byte[] v1 = reg_read_vector(o1.value.reg);
                    byte[] v2 = reg_read_vector(o2.value.reg);
                    pointer.write(0, v1, 0, v1.length);
                    pointer.write(16, v2, 0, v2.length);
                } else {
                    throw new UnsupportedOperationException();
                }
                if (opInfo.writeback) {
                    Arm64.Operand mem = opInfo.op[2];
                    reg_write(mem.value.reg, vaddr);
                }
                hypervisor.reg_set_elr_el1(elr + 4);
                return true;
            }
