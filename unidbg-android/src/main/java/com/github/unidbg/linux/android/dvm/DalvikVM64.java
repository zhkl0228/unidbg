        ...
        Pointer _CallStaticLongMethodA = svcMemory.registerSvc(new Arm64Svc() {
            ...
            public long handle(Emulator<?> emulator) {
                ...
                if (dvmMethod == null) {
                    ...
                    if (verbose) {
                        System.out.printf("JNIEnv->CallStaticLongMethodA(%s, %s(%s) => 0x%xL) was called from %s%n", dvmClass, dvmMethod.methodName, vaList.formatArgs(), ret, context.getLRPointer());
                    }
                    return ret;
                }
            }
